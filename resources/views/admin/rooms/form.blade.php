    ...
    <div class="row g-3">
        ...
        <div class="col-md-8">
            ...
            <input type="text" name="subtitle" class="form-control"
                value="{{ old('subtitle', $room->subtitle ?? '') }}">
        </div>
        <div class="col-md-4">
            <label class="form-label">Superficie</label>
            <input type="text" name="external_id" class="form-control"
                value="{{ old('external_id', $room->external_id ?? '') }}" placeholder="Ex: APT-102">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-12">
            ...
            <textarea name="description" class="form-control js-tinymce-room"
                rows="10">{{ old('description', $room->description ?? '') }}</textarea>
        </div>
        ...
        <div class="col-md-6">
            ...
            <textarea name="checkin_info" class="form-control" rows="4"
                placeholder="">{{ old('checkin_info', $room->checkin_info ?? '') }}</textarea>
        </div>
        <div class="col-md-6">
            <label class="form-label">Check-out <small class="text-muted">(une règle par ligne)</small></label>
            <textarea name="checkout_info" class="form-control" rows="4"
                placeholder="">{{ old('checkout_info', $room->checkout_info ?? '') }}</textarea>
        </div>
        <div class="col-md-6">
            <label class="form-label">Instructions spéciales check-in</label>
            <textarea name="special_instructions" class="form-control" rows="4"
                placeholder="">{{ old('special_instructions', $room->special_instructions ?? '') }}</textarea>
        </div>
        <div class="col-md-6">
            <label class="form-label">Enfants &amp; lits supplémentaires</label>
            <textarea name="children_policy" class="form-control" rows="4"
                placeholder="">{{ old('children_policy', $room->children_policy ?? '') }}</textarea>
        </div>
        <div class="col-12">
                    ...
                    <div class="col-md-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="amenities[]" value="{{ $amenity->id }}"
                                id="amenity_{{ $amenity->id }}" {{ $checked ? 'checked' : '' }}>
            ...
            <div class="d-flex gap-3 align-items-start">
                ...
                <div id="main-image-preview-wrap" class="flex-grow-1">
                    ...
                    <div id="main-image-new-wrap" class="d-none">
                        <img id="main-image-new" src="" alt="" style="width:100%;max-height:160px;object-fit:cover;"
                            class="rounded shadow-sm">
                        <div class="text-primary small mt-1"><i class="bi bi-check2-circle"></i> Nouvelle image
                            sélectionnée – sera redimensionnée à 1550×1080 à l'enregistrement</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-12">
                ...
                <div id="gallery-sortable" class="d-flex flex-wrap gap-2 mb-3">
                    @foreach($room->gallery as $img)
                        <div class="gallery-item position-relative" data-path="{{ $img }}">
                            ...
                            <button type="button"
                                class="btn btn-sm btn-danger position-absolute top-0 end-0 gallery-remove-btn shadow-sm"
            ...
            <small class="text-muted mt-2 d-block">Glissez les images existantes pour réordonner. Cliquez ✕ pour
                supprimer.</small>
        </div>
        <div class="col-md-4">
            <label class="form-label">Statut</label>
            <select name="status" class="form-select">
                <option value="draft" {{ old('status', $room->status ?? 'draft') === 'draft' ? 'selected' : '' }}>
                    Brouillon</option>
                <option value="published" {{ old('status', $room->status ?? 'draft') === 'published' ? 'selected' : '' }}>
                    Publié</option>
            </select>
        </div>
    </div>
